...
int OSFFWriteTask::run()
{
    ...
    if ((this->status = this->pop_pkt_cb(this->osff_fmt_ctx->rw_pkt_queue_id,
                                         (void *&)pkt,
                                         THREAD_NO_TIMEOUT)) < 0)
    {
    ...
    if ((this->status = av_interleaved_write_frame(this->osff_fmt_ctx->fmt_ctx,
                                                   pkt)) < 0)
    {
        av_log(this->osff_fmt_ctx->fmt_ctx, AV_LOG_ERROR,
    ...
    if (pkt == nullptr)
    {
        ...
        if (this->num_nullptr == this->osff_fmt_ctx->fmt_ctx->nb_streams)
        {
...
int OSFFWriteTask::write_header()
{
    if (this->osff_fmt_ctx->fmt_ctx->oformat->flags & AVFMT_NOFILE)
    {
        av_log(this->osff_fmt_ctx->fmt_ctx, AV_LOG_ERROR,
    ...
    {
        if ((this->status = avformat_write_header(this->osff_fmt_ctx->fmt_ctx,
                                                  nullptr)) < 0)
        {
            av_log(this->osff_fmt_ctx->fmt_ctx, AV_LOG_ERROR,
...
int OSFFWriteTask::write_tailer()
{
    if (this->osff_fmt_ctx->fmt_ctx->oformat->flags & AVFMT_NOFILE)
    {
        av_log(this->osff_fmt_ctx->fmt_ctx, AV_LOG_ERROR,
    ...
    {
        if ((this->status = av_write_trailer(this->osff_fmt_ctx->fmt_ctx)) < 0)
        {
            av_log(this->osff_fmt_ctx->fmt_ctx, AV_LOG_ERROR,
