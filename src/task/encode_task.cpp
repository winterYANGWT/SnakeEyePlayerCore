...
int SnakeEye::SnakeEyeEncodeTask::run()
{
    // pop a frame from the stream frame queue
    AVFrame *frm = nullptr;

    if ((this->status = this->pop_frm_cb(this->se_strm_ctx->strm_frm_queue_id,
                                         (void *&)frm,
                                         THREAD_NO_TIMEOUT)) < 0)
    {
    ...
    if ((this->status = avcodec_send_frame(this->se_strm_ctx->codec_ctx,
                                           frm)) < 0)
    {
    ...
    while (!this->exit_flag)
    {
        ...
        if ((this->status = avcodec_receive_packet(this->se_strm_ctx->codec_ctx,
                                                   pkt)) < 0)
        {
        ...
        if ((this->status = this->push_pkt_cb(this->se_strm_ctx->strm_pkt_queue_id,
                                              pkt,
                                              THREAD_NO_TIMEOUT)) < 0)
        {
            av_log(nullptr, AV_LOG_ERROR,
